...
#define UART_MEM_SIZE	0x10

void Init_UART_PinMux(CHIP_SWM_PIN_MOVABLE_T UART_TX, uint8_t tx_pin, CHIP_SWM_PIN_MOVABLE_T UART_RX, uint8_t rx_pin);
void setupUART(uint32_t UART_BASE, UART_HANDLE_T* uartHandle, uint32_t uartHandleMEM[UART_MEM_SIZE],uint8_t mem_size, UART_CONFIG_T* cfg);
